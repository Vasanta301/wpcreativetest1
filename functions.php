...
function wpc_custom_enqueue_resources() {
  ...
  wp_enqueue_script( 'wpc-custom-js', $js_dir.'/wpc-custom.js', ['jquery'], '', time() );
  wp_enqueue_style( 'wpc-style', $css_dir.'/wpc-style.css', time() );
  wp_localize_script( 'wpc-custom-js', 'wpcAjaxObject',
	array( 
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'ajaxnonce' => wp_create_nonce('wpcAjaxNonce')
	)
);
}
...
function wpc_filter()
{
	...
	$args['order'] = sanitize_text_field($param['order'])??'ASC';
	$args['orderby'] = 'title';
    ...
    if ($the_query->have_posts()) :
        ...
        echo '<h2>No post found. Please try other combination.</h2>';
    endif;
...
add_action('wp_ajax_wpc_filter', 'wpc_filter');
add_action('wp_ajax_nopriv_wpc_filter', 'wpc_filter');
...
function custom_user_registration() {
    ...
    if ( isset( $_POST['username'] ) && isset( $_POST['email'] ) && isset( $_POST['password'] ) ) {
        ...
        if ( ! validate_username( $username ) ) {
            $error_message = urlencode( 'Invalid username' );
            wp_redirect( home_url( '/register/?message=1' ) );
            exit;
        }
        if ( ! is_email( $email ) ) {
            $error_message = urlencode( 'Invalid email address' );
            wp_redirect( home_url( '/register/?message=2' ) );
            exit;
        }
        if ( username_exists( $username ) ) {
            $error_message = urlencode( 'Username already exists' );
            wp_redirect( home_url( '/register/?message=3' ) );
            exit;
        }
        if ( email_exists( $email ) ) {
            $error_message = urlencode( 'Email address already exists' );
            wp_redirect( home_url( '/register/?message=4' ) );
            exit;
        }
 ...
 * @return void
 */
function trigger_after_user_is_registered( $user_id, $user_data ){
		$request = wp_remote_post(
			'http://localtest.local/wp-json/wp/v2/user',
			array(
				'headers' => array(
...
add_action('user_register','trigger_after_user_is_registered',10,2);